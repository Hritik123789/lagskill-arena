...
import { User, Activity, Target, Clock, TrendingUp, BarChart3, Shield, Award, Zap, AlertTriangle, CheckCircle } from 'lucide-react';
...
export function DashboardPage() {
  ...
  const getPerformanceTier = (score: number) => {
    if (score >= 90) return { name: 'Diamond', color: 'cyan', icon: '💎', min: 90, max: 100 };
    if (score >= 75) return { name: 'Platinum', color: 'purple', icon: '🏆', min: 75, max: 90 };
    if (score >= 60) return { name: 'Gold', color: 'yellow', icon: '🥇', min: 60, max: 75 };
    if (score >= 40) return { name: 'Silver', color: 'gray', icon: '🥈', min: 40, max: 60 };
    return { name: 'Bronze', color: 'orange', icon: '🥉', min: 0, max: 40 };
  };

  const currentTier = getPerformanceTier(avgPerformanceScore);
  const progressToNextTier = currentTier.max > 0 
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-sm font-medium text-gray-400">Avg Score</h3>
            </div>
          ...
          <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6">
            ...
            <p className="text-3xl font-bold text-white">
              {avgPerformanceScore >= 80 ? 'Gold' : avgPerformanceScore >= 60 ? 'Silver' : 'Bronze'}
            </p>
          </div>
        </div>

        {/* AI Verdict Card - THE CORE DIFFERENTIATOR */}
